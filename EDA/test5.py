...
#             for box in boxes:
#                 cls_id = int(box.cls)
#                 label = class_names[cls_id]  # 클래스 ID를 클래스 이름으로 변환
...
#         yolo_roi = frame[roi_y_large:roi_y_large + roi_height_large, roi_x_large:roi_x_large + roi_width_large]
#         results = model(yolo_roi)
#         annotated_frame = results[0].plot()  # 주석 달린 프레임 생성
...
#                 cv2.putText(frame, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
#                 trash_detected = True
#                 convert_to_robot_coordinates((x1, y1, x2, y2), class_name)
...
#         results = model(yolo_roi)
#         for result in results:
#             annotated_frame = result.plot()  # 주석 달린 프레임 생성
