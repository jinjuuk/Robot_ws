...
def detect_star_shape(image, canny_thresh1, canny_thresh2):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) # 인식된 이미지를 gray 컬러로 변환
    blurred = cv2.GaussianBlur(gray, (5, 5), 0) # 배경의 노이즈를 제거하기 위해 블러 처리(외각선 따기 편리함)
    edged = cv2.Canny(blurred, canny_thresh1, canny_thresh2) # canny 알고리즘은 엣지검출하는 
